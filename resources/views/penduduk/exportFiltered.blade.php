...
<head>
    <meta charset="UTF-8">
    <title>Data Warga @if ($rt && $rw)RT {{ $rt->rt }} / RW {{ $rw->rw }} @elseif ($rw) RW {{ $rw->rw }} @elseif ($rt) RT {{ $rt->rt }} @endif Kelurahan Kampung Bulang @if (isset($tahun)) {{ $tahun }} @endif
    </title>
...
<body>
    ...
    <div class="mb-3">
        <p>
            Berikut ini adalah data lengkap warga yang berdomisili di wilayah Kelurahan Kampung Bulang khususnya di
            wilayah
            @if ($rt && $rw)
                RT {{ $rt->rt }} / RW {{ $rw->rw }}
            @elseif ($rw)
            ...
            @foreach ($penduduk as $pd)
                <tr>
                    ...
                    <td>
                        ...
                        {{ $statusEkonomi }}
                        <br>
                        <small class="text-muted">Rp {{ number_format($rataRata, 0, ',', '.') }}
                            ({{ number_format($rasio, 2) }})</small>
                    </td>
                </tr>
            @endforeach
        </tbody>
